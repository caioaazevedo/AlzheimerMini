...
@IBDesignable public class SHCircleBar: UITabBar {
    ...
    private func drawPath(for index: CGFloat) -> UIBezierPath {
        ...
        let firstPointFirstCurve = CGPoint(x: ((tabWidth * index) + tabWidth / 2), y: 0)
        let firstPointSecondCurve = CGPoint(x: ((index * tabWidth) - 25) + tabWidth / 8, y: 42)
        
        let middlePoint = CGPoint(x: (tabWidth * index) + tabWidth / 2, y: 55)
        let middlePointFirstCurve = CGPoint(x: (((tabWidth * index) + tabWidth) - tabWidth / 8) + 25, y: 42)
        let middlePointSecondCurve = CGPoint(x: (((tabWidth * index) + tabWidth) - tabWidth / 2), y: 0)
